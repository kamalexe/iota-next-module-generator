...
export default config;
`;
function writeInitConfig(configPath) {
    const absolutePath = path.resolve(configPath);
    if (fs.existsSync(absolutePath)) {
        console.error(`❌ ${configPath} already exists.`);
        process.exit(1);
    }
    fs.writeFileSync(absolutePath, INIT_CONFIG_TEMPLATE.trimStart(), "utf8");
    console.log(`✓ Created ${configPath}`);
    console.log("Edit it to match this project, then run the generator again.");
}
...
async function loadConfig(configPath) {
    ...
    try {
        ...
        if (!raw) {
            console.warn(`⚠️ ${configPath} has no default export. Using defaults.`);
            return DEFAULT_CONFIG;
        }
        return mergeConfig(DEFAULT_CONFIG, raw);
    }
    catch (error) {
        console.warn(`⚠️ Could not load ${configPath}:`, error instanceof Error
            ? error.message
            : error);
 ...
 *   "../../models/task.model"
 */
function makeImporter(alias, srcDirName) {
    if (!alias) {
        return function (target, fromFile) {
    ...
    return function (target, _fromFile) {
 ...
 */
function resolveImportPrefix(configuredAlias, srcDirName, tsconfigPath) {
    ...
    try {
        ...
        const pattern = new RegExp(`"${escapeRegExp(configuredAlias)}\\*"\\s*:\\s*\\["([^"]+)"`);
...
function ensureTsConfigAlias() {
    ...
    if (!fs.existsSync(tsconfigPath)) {
        console.warn("⚠️ tsconfig.json not found. Skipping @/* alias setup.");
        return;
    }
    let content = fs.readFileSync(tsconfigPath, "utf8");
    // Already configured
    if (/"@\/*"\s*:\s*\[\s*"\.\/src\/\*"\s*\]/.test(content)) {
        console.log('✓ TypeScript alias "@/*" already configured');
        return;
    }
    // If paths already exists
    if (/"paths"\s*:\s*\{/.test(content)) {
        content = content.replace(/("paths"\s*:\s*\{)/, `$1\n      "@/*": ["./src/*"],`);
    ...
    console.warn("⚠️ Could not configure @/* alias automatically.");
}
/**
 * =========================================================
 * MODEL
 ...
 * =========================================================
 */
function repositoryTemplate(modelImport) {
    return `
import {
    ${ModuleName}Model,
} from "${modelImport}";
 ...
 * =========================================================
 */
function apiCollectionTemplate(moduleImport) {
    return `
import {
    NextRequest,
    NextResponse,
 ...
 * =========================================================
 */
function apiSingleTemplate(moduleImport) {
    return `
import {
    NextRequest,
    NextResponse,
...
async function main() {
    ...
    const srcDirName = path.basename(path.resolve(config.paths.srcDir));
    const modulesDir = path.resolve(config.paths.modulesDir);
    const modelsDir = path.resolve(config.paths.modelsDir);
    const apiDir = path.resolve(config.paths.apiDir);
    const modulePath = path.join(modulesDir, moduleName);
    const modelPath = path.join(modelsDir, `${moduleName}.model.ts`);
    const apiPath = path.join(apiDir, collectionName);
    /* ---- Import prefix ---- */
    const configuredAlias = cli.alias === "none"
        ? null
        : cli.alias ?? config.alias ?? null;
    let alias = null;
    if (configuredAlias) {
        alias = resolveImportPrefix(configuredAlias, srcDirName, config.tsconfigPath);
    }
    const importer = makeImporter(alias, srcDirName);
    const modelImport = importer(path.join(modelsDir, `${moduleName}.model`), modulePath);
    const moduleImport = importer(path.join(modulesDir, moduleName), apiPath);
    /* ---- Existing file checks ---- */
    if (!cli.force) {
        if (fs.existsSync(modulePath)) {
            console.error(`❌ Module already exists: ${path.relative(root, modulePath)}`);
            console.error("Use --force to overwrite it.");
            process.exit(1);
        }
        if (fs.existsSync(modelPath)) {
            console.error(`❌ Model already exists: ${path.relative(root, modelPath)}`);
            console.error("Use --force to overwrite it.");
            process.exit(1);
        }
        if (fs.existsSync(apiPath)) {
            console.error(`❌ API already exists: ${path.relative(root, apiPath)}`);
            console.error("Use --force to overwrite it.");
            process.exit(1);
        }
    }
    ...
    console.log(`  📁 modules: ${path.relative(root, modulesDir)}`);
    console.log(`  📁 models:  ${path.relative(root, modelsDir)}`);
    console.log(`  🌐 api:     ${path.relative(root, apiDir)}`);
    console.log(alias
        ? `  🔧 alias:    "${alias}" (imports use this prefix)`
        : "  🔧 alias:    relative imports (no alias)");
    /* ---- Generate ---- */
    generateFile(path.join(modulePath, "index.ts"), indexTemplate);
    generateFile(path.join(modulePath, `${collectionName}.repository.ts`), repositoryTemplate(modelImport));
    generateFile(path.join(modulePath, `${collectionName}.service.ts`), serviceTemplate);
    generateFile(path.join(modulePath, `${collectionName}.schema.ts`), schemaTemplate);
    generateFile(path.join(modulePath, "serializer.ts"), serializerTemplate);
    generateFile(path.join(modulePath, "types.ts"), typesTemplate);
    generateFile(path.join(modulePath, "actions.ts"), actionsTemplate);
    generateFile(modelPath, modelTemplate);
    generateFile(path.join(apiPath, "route.ts"), apiCollectionTemplate(moduleImport));
    generateFile(path.join(apiPath, "[id]", "route.ts"), apiSingleTemplate(moduleImport));
    /* ---- Done ---- */
    console.log("");
    console.log(`✅ ${ModuleName} module generated successfully!`);
    console.log("");
    console.log("Generated:");
    console.log(`  📦 ${path.relative(root, modelPath)}`);
    console.log(`  📦 ${path.relative(root, modulePath)}/`);
    console.log(`  🌐 ${path.relative(root, apiPath)}/`);
    console.log("");
    console.log(`API: /api/${collectionName}`);
    console.log("");
}
main().catch((error) => {
    console.error(error);
